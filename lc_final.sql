...
LOAD DATA LOCAL INFILE 'C:\\Users\\Nicolas\\Documents\\Work\\SQL\\Post-COVID_Conditions_new.csv' 
...
-- Note that time_period_start AND time_period_end dates are in non-standard MySQL date format, MySQL: YYYY-MM-DD

-- Transforming the data type for the date columns from the VARCHAR to DATE datatype and standardizing the date format
...
-- WHERE time_period_start = '2022-09-14' -- arbitrary value
-- AND indicator = 'Ever had COVID' -- arbitrary value
WHERE post_covid_conditions.group <> 'By State' -- interested only in national long covid rates per demographic, not by state
;
...
-- The Subgroup column in the table contains all the demographic information needed for analysis. However, the current formatting of the data in the table makes this difficult to read

-- Creating a temporary table to transform the subgroup table into discrete categories of age, sex, gender identity, sexuality, race, education level, and disability status using CASE statements with corresponding percentages of Long Covid rates
...
SELECT * FROM long_covid_groups;

-- DROP TABLE long_covid_groups;

-- Create a temporary table showing Long Covid rates for all demographics per Long Covid indicator, grouped by survey start date
...
FROM long_covid_groups
GROUP BY 1, 2
ORDER BY survey_start_date;

SELECT * FROM indicator_level 
WHERE survey_start_date = '2022-09-14' -- arbitrary value
AND indicator IN ('Currently experiencing long COVID, as a percentage of adults who ever had COVID', 'Currently experiencing long COVID, as a percentage of all adults'); -- arbitrary value
...
# 2022-09-14	Currently experiencing long COVID, as a percentage of adults who ever had COVID	15.00	11.00	14.00	16.00	18.00	17.00	16.00	15.00	12.00	18.00	15.00	30.00	15.00	16.50	15.00	15.00	14.00	9.00	19.00	18.00	12.00	30.00	13.00
# 2022-09-14	Currently experiencing long COVID, as a percentage of all adults	7.00	6.00	8.00	9.00	9.00	7.00	5.00	5.00	6.00	9.00	7.00	17.00	7.00	8.50	8.00	7.00	6.00	4.00	10.00	8.33	6.00	14.00	6.00

-- Now, percentage values of Long Covid prevalence per demographic are grouped by indicator and survey start date for better readability and mimics pivot table views in Excel

-- Finally, creating a new table in the database with the cleaned and transformed data for further analysis
CREATE TABLE indicator_level
SELECT * FROM lc_indicator_level;